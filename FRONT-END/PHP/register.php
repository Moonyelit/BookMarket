    ...
    <div class="bg-brown p-8 rounded-3xl shadow-xl w-full max-w-lg">
      ...
      <form action="../../BACK-END/process/userInscription.php" method="POST" class="space-y-4" enctype="multipart/form-data">
        ...
        <div id="additional-fields" class="space-y-4">
          <?php if (isset($_GET['role']) && ($_GET['role'] === 'vendeur' || $_POST['role'] === 'les-deux')): ?>
            <div id="additional-fields" class="space-y-4">
              <?php if (isset($_GET['role']) && ($_GET['role'] === 'vendeur' || $_GET['role'] === 'les-deux')): ?>
                <div>
                  <label for="entreprise" class="block text-yellow mb-1">Nom de l'entreprise</label>
                  <input type="text" id="entreprise" name="entreprise" class="w-full p-2 rounded bg-white text-black" maxlength="30">
                </div>
                <div>
                  <label for="telephone-vendeur" class="block text-yellow mb-1">Numéro de téléphone</label>
                  <input type="tel" id="telephone-vendeur" name="telephone-vendeur" class="w-full p-2 rounded bg-white text-black" maxlength="30">
                </div>
                <div>
                  <label for="adresse-vendeur" class="block text-yellow mb-1">Adresse</label>
                  <input type="text" id="adresse-vendeur" name="adresse-vendeur" class="w-full p-2 rounded bg-white text-black" maxlength="30">
                </div>
                <div>
                  <label for="code-postal-vendeur" class="block text-yellow mb-1">Code postal</label>
                  <input type="text" id="code-postal-vendeur" name="code-postal-vendeur" class="w-full p-2 rounded bg-white text-black" maxlength="30">
                </div>
                <div>
                  <label for="pays-vendeur" class="block text-yellow mb-1">Pays</label>
                  <input type="text" id="pays-vendeur" name="pays-vendeur" class="w-full p-2 rounded bg-white text-black" maxlength="30">
                </div>
              <?php endif; ?>
            </div>
            <div>
      ...
      <p class="text-yellow mt-4 text-center">
        Déjà un compte ? <a href="login.html" class="underline">Connectez-vous</a>
      </p>
    </div>
